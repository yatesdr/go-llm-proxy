...
- **Admin Processors page** at `/admin/processors`. Global configuration for vision, audio, and OCR processor models plus the web search API key, via dropdowns wired to the model list (vision dropdown filters to `supports_vision: true`; audio to `supports_audio: true`). Single-key web search for now — multi-provider cascading (Brave/Tavily fall-through) is tracked as follow-up. Routes registered in `main.go`; new tab in the admin nav.
- **`supports_audio` field** on `ModelConfig`, mirroring `supports_vision`. Surfaces as a purple "audio" pill on the model status page alongside the existing "vision" pill. Auto-inferred when a model is named as the global `processors.audio`.
- **Audio processor slot in `ProcessorsConfig`** (new `audio` field). The config value is validated, persisted via `UpdateProcessors`, and carried through reference tracking (`modelReferrers` / `stripModelReferences`). The actual audio-transcription pipeline stage that consumes the field is scheduled for v0.3.9 — the admin page surfaces a hint noting this.
- **Three-tier data-safety classification** on the model status page. Replaces the binary Safe/Warning badges with: 🟢 Local — Safe for data (self-hosted on private IP), 🟢 Bedrock — Safe for data (AWS Zero-Operator-Access; model provider walled off from prompts), 🟡 3rd Party — Warning (direct to model provider). Each tier has a tooltip explaining the underlying guarantee.
- **OpenCode GUI install path** on Windows. Generator now calls out that TUI uses `%APPDATA%\opencode\opencode.json` but GUI uses `%USERPROFILE%\.config\opencode\opencode.json`; a misplaced config is why the GUI silently failed to reach the proxy for one user.
- **`poppler-utils` in the Docker image** to enable `pdftoppm` rasterization for the PDF OCR pipeline.
...
- **Pipeline output uses XML-like tags** so target models don't conflate pipeline-injected content with user-authored text. `[Image description: ...]` → `<image_description>...</image_description>`; `[Page text: ...]` → `<page_text>...</page_text>`; `[PDF: foo.pdf]\n\n...` → `<pdf_content filename="foo.pdf" source="text|ocr|vision">\n...\n</pdf_content>`. Failure strings are unchanged. Breaking for downstream consumers that parsed the bracket format — no public schema involved, but operators with custom logging on these strings should update.
- **Context-window detection precedence flipped.** Backend-detected values now win over configured values (the live backend is more authoritative than a hand-edit). Config-provided `context_window` is now the fallback, used only when detection errors or returns zero. Previously detection was skipped entirely when config had a value set, so a stale config permanently masked the real backend capability.
- **Model-edit admin form restructured.** Required fields (name, type, backend, upstream model, api_key) grouped at the top under a blue "Required" heading; dashed separator; optional sections ordered by likelihood of needing configuration: Capabilities → Bedrock credentials → Protocol overrides → Sampling defaults → Per-model processors. Every tooltip updated to document the default value and explain when you'd change the field.
- **Admin table layout rebuilt.** Switched to `table-layout:fixed` with explicit column widths on every `<th>`; action-button cell no longer applies `display:flex` directly to `<td>` (which broke table-row baseline alignment) — flex now lives on an `.action-group` div inside the cell; uniform 48px row height; shared container widened from 960px to 1200px. Fixes cross-page row-height drift, Delete-button clipping, and Models-page horizontal overflow on the 8-column layout.
- **Secret rotation is non-destructive when input is blank.** On both the model-edit form and the processors page, clicking Rotate then Save with an empty input is now a no-op (previously it sent `""` to the backend and wiped the stored value). The explicit Clear button remains the only way to delete a secret. Input placeholders updated to make the semantics obvious at a glance.
...
- **Bedrock models missing `context_window` in `/v1/models`.** Non-lookup-table Bedrock models (`zai.glm-4.7`, `moonshot.kimi-k2-thinking`) were dropped from the response because detection returned 0 and never wrote a value; the config-provided override was ignored because the old code path treated the config value as a reason to *skip* detection entirely. The new precedence (see Changed) preserves config values as fallback when detection returns zero.
- **JS SyntaxError breaking the admin Users and Models pages.** Go raw-string literals `\\'` produced `\\'` in the emitted JS, which the browser parsed as escaped-backslash + string-terminator — a parse error on `newKeyVal\'` that stopped the admin JS bundle from executing, leaving the table stuck on "Loading…". Corrected to `\'` in `admin_users.go:222` and `admin_models.go:571-572`.
- **Flaky Bedrock context-window lookup.** `lookupBedrockContextWindow` iterated the map until the first prefix match; map iteration is random in Go, so `cohere.command-r-plus-v1:0` could match `cohere.command` (4000) before `cohere.command-r-plus` (128000). Switched to longest-prefix-match so the result is deterministic.
- **Scanned-PDF extraction broken via Anthropic Messages.** `processPDFs` picked exactly one fallback model (OCR if configured, else vision) and called it once. Since paddleOCR-style OCR backends reject raw `data:application/pdf` input, every scanned PDF silently failed. Replaced with a three-stage cascade: native text extraction → rasterize PDF to PNG pages → send each page to dedicated OCR model → vision fallback with raw PDF. Rasterization uses `pdftoppm` (poppler-utils) or `gs` (Ghostscript) if available; if neither is installed, stage 2 is skipped and the vision model handles scanned PDFs directly.
- **Tool-role image OCR failure was terminal.** Parallel fix in `processImages`: tool-role images (Codex `view_image` output, screenshots) now cascade OCR → vision. User-role natural photos keep the vision-only path — dedicated OCR models hallucinate on photographs and this is documented in the code.
...
- Live-tested end-to-end against AWS Bedrock (`zai.glm-4.7-flash` on-demand + `us.amazon.nova-lite-v1:0` inference profile, us-east-2) across all four client × stream-mode combinations plus tool calls.
- Empirically confirmed `/v1/models` now reports `context_window` for `zai.glm-4.7` (202752) and `moonshot.kimi-k2-thinking` (262144) from the configured values.
