...
A single-binary LLM proxy targeting compatibility and native behaviors with common coding agents such as Claude Codex, Codex, OpenCode, Qwen, and Claw.    It proxies as you'd expect a proxy to for local models and upstream providers, ranslates between API protocols, routes requests across backends, and runs upstream tools locally to enable commonly used features for local back-ends like llama-server and vllm.  Adds web search (via Tavily key), and routes vision requests to a specified vision model to give you full capability in your harness tooling.
